"""Honest ephemeral-CoT (driver, not committed): think-blocks as throwaway scratchpad.

Fixes the leak found in cot_50m.py (past <think> blocks rebuilt at inference via the consistency
filter -> contamination). Here the <think> is EPHEMERAL: each turn is its own training example whose
history is BOARD-ONLY (<GUESS> guess + feedback + <SEP>, NO think). The model regenerates fresh
reasoning each turn from the board, commits a guess, and the reasoning is discarded — never enters the
context for later turns. Training teacher builds the target think (consistent candidates), exactly like
InfoMax-teacher SFT; at inference the model reasons in its OWN weights with ZERO rules and the filter
is never called. Train and inference are now identical. Held-out discipline. -> runs/cot_eph.pt.
"""
...
from wordle_slm.sft.train import _batches, load_checkpoint, save_checkpoint
...
def board_only(turns):
    """Past turns as board history ONLY — guess + feedback + sep, NO think-blocks (ephemeral)."""
    ids = [tok.bos_id]
    for turn in turns:
...
def game_examples(game, rng):
    """One training example per turn: board-only history (no loss) + current think+guess (loss)."""
    exs = []
...
def play(model, secret):
    """Honest play: board-only prompt; model generates fresh think+guess; think is discarded."""
    g = Game(secret)
...
print(f"      {len(games)} games -> {len(exs)} per-turn examples; ex len~{len(exs[0][0])}", flush=True)
...
for epoch in range(EPOCHS):
    ...
    if epoch % 3 == 0 or epoch == EPOCHS - 1:
        ...
        if m["win"] > best:
            best = m["win"]
            save_checkpoint("runs/cot_eph.pt", model, opt, epoch, SFTConfig())
            flag = "  <- best, saved"
        print(f"  epoch {epoch:>2}  loss={float(loss.detach()):.3f}  win={m['win']:.3f} valid={m['valid']:.3f} avg={m['avg']:.2f}{flag}", flush=True)
    model.train()

print(f"\n=== HONEST ephemeral-CoT: full held-out ({len(held)}), best checkpoint ===", flush=True)
b = WordleGenerator(CFG, VOCAB).to(DEV)
load_checkpoint("runs/cot_eph.pt", b)
f = evaluate(b, tuple(held))
print(f"  held-out win : {f['win']:.3f}  ({int(round(f['win'] * len(held)))}/{len(held)})   [char-50M+aux = 0.436; leaked-CoT honest = 0.192]", flush=True)

...
def pick_cands(history, guess, rng):
    cons = [w for w in consistent_candidates(history, CAND_POOL) if w != guess]  # FM-2 fix: TRAIN-only
    rng.shuffle(cons)
...
CAND_POOL = tuple(train)  # FM-2 fix: candidates drawn from TRAIN only (never surface a held-out word)
...
curve = VAL
print(f"[clean] openers={SAFE_OPENERS}  |VAL|={len(VAL)} |TEST|={len(TEST)} (disjoint)  cand-pool=train", flush=True)
...
games = []
for s in range(5):
    games += [tr.game for tr in generate_transcripts(tuple(train), weak_frac=0.2, openers=SAFE_OPENERS, seed=s, valid_pool=tuple(train), answer_pool=tuple(train))]  # teacher TRAIN-only (no held-out guesses)
rng = Random(0)
...
for epoch in range(EPOCHS):
    ...
    if epoch % 5 == 0 or epoch == EPOCHS - 1:
        ...
        if m["win"] > best:
            best = m["win"]
            save_checkpoint("runs/cot_eph_aux_clean.pt", model, opt, epoch, SFTConfig())
            flag = "  <- best, saved"
...
print("\n=== CLEAN ephemeral-CoT + aux: honest eval (selected on VAL, reported on disjoint TEST) ===", flush=True)
b = WordleGenerator(CFG, VOCAB).to(DEV)
load_checkpoint("runs/cot_eph_aux_clean.pt", b)
ft = evaluate(b, TEST)
fv = evaluate(b, VAL)
fa = evaluate(b, tuple(held))
...
print(f"  full  held (reference, ⊇VAL)      : win {fa['win']:.3f}   [contaminated 0.616 was here]", flush=True)
print("  ^ delta(full−TEST)=selection bias; delta(this TEST vs the contaminated 0.616)=total leak", flush=True)

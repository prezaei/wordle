...
PROG = "runs/cot_eph_aux_fair_progress.jsonl"  # per-epoch boards+metrics -> scripts/live_viz.py
if os.path.exists(PROG):
    os.remove(PROG)  # start each run clean (don't mix epochs across runs)
print(f"[fair] openers={SAFE_OPENERS} |VAL|={len(VAL)} |TEST|={len(TEST)}  cand-pool=DICTIONARY(valid {len(VALID)})", flush=True)
...
for epoch in range(EPOCHS):
    ...
    if epoch % 5 == 0 or epoch == EPOCHS - 1:
        ...
        if m["win"] > best:
            best = m["win"]
            save_checkpoint("runs/cot_eph_aux_fair.pt", model, opt, epoch, SFTConfig())
            flag = "  <- best, saved"
...
load_checkpoint("runs/cot_eph_aux_fair.pt", b)
ft = evaluate(b, TEST)
fv = evaluate(b, VAL)

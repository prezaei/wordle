...
held-out greedy, no rules. Starts from runs/dpo.pt (0.631) -> runs/rl_grpo_guessonly.pt.
"""
...
def guess_only_mask(seq):
    """Action mask True ONLY on the 5 committed guess letters (after the last <GUESS>) — credit the
    COMMIT, not the <think> scratchpad (isolates the commit; tests the think-dilution question)."""
    m = [False] * len(seq)
...
def rollout_batch(model, secrets, rows=ROWS):
    ...
    for _ in range(rows):
        ...
        for j, i in enumerate(active):
            ...
            seq = prompts[j] + gen[j]
            records[i].append((seq, guess_only_mask(seq)))  # GUESS-ONLY credit
...
rngS.shuffle(order)
best = b10["win"]
save_checkpoint("runs/rl_grpo_guessonly.pt", model, opt, 0, SFTConfig())
ptr = 0
for u in range(UPDATES):
    ...
    if u % 8 == 0 or u == UPDATES - 1:
        ...
        if e10["win"] > best:
            best = e10["win"]
            save_checkpoint("runs/rl_grpo_guessonly.pt", model, opt, u + 1, SFTConfig())
            flag = "  <- best, saved"
...
load_checkpoint("runs/rl_grpo_guessonly.pt", b)

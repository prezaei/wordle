...
teacher); inference stays greedy ephemeral-CoT with ZERO rules. Base runs/cot_eph_aux.pt -> runs/dpo.pt.
"""
...
curve = tuple(held[:96])
print(f"[load] runs/cot_eph_aux.pt (0.616 base)  beta={BETA} N_roll={N_ROLL} secrets={N_SECRETS}", flush=True)
...
best = base["win"]
save_checkpoint("runs/dpo.pt", policy, opt, 0, SFTConfig())
EPOCHS, BATCH = 4, 16
for ep in range(EPOCHS):
    ...
    if m["win"] > best:
        best = m["win"]
        save_checkpoint("runs/dpo.pt", policy, opt, ep + 1, SFTConfig())
        flag = "  <- best, saved"
    else:
        load_checkpoint("runs/dpo.pt", policy)

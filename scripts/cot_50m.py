"""Scaled CoT (driver, not committed): 50M + chain-of-thought, the honest path to a new best.

Scales the winning CoT prototype (14M -> 0.415) to 50M. The model reasons (enumerates consistent
candidates) then commits a guess, learned from teacher traces; ZERO rules at inference. Held-out
discipline (train on train answers, eval on held). Best-by-held-out -> runs/cot_50m.pt.
"""
...
print("[data] teacher games + CoT traces …", flush=True)
games = []
for s in range(5):
...
print(f"      {len(games)} games; cot len~{len(seqs[0][0])}", flush=True)
...
for epoch in range(EPOCHS):
    ...
    if epoch % 4 == 0 or epoch == EPOCHS - 1:
        ...
        if m["win"] > best:
            best = m["win"]
            save_checkpoint("runs/cot_50m.pt", model, opt, epoch, SFTConfig())
            flag = "  <- best, saved"
        print(f"  epoch {epoch:>2}  loss={float(loss.detach()):.3f}  win={m['win']:.3f} valid={m['valid']:.3f} avg={m['avg']:.2f}{flag}", flush=True)
    model.train()

print(f"\n=== CoT-50M milestone: full held-out ({len(held)}), best checkpoint ===", flush=True)
b = WordleGenerator(CFG, VOCAB).to(DEV)
load_checkpoint("runs/cot_50m.pt", b)
f = evaluate(b, tuple(held))
print(f"  held-out win : {f['win']:.3f}  ({int(round(f['win'] * len(held)))}/{len(held)})   [char-50M+aux = 0.436]", flush=True)

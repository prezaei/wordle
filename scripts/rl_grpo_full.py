...
held-out greedy, no rules. Starts from runs/dpo.pt (0.631) -> runs/cot_eph_aux_grpo.pt.
"""
...
rngS.shuffle(order)
best = b10["win"]
save_checkpoint("runs/cot_eph_aux_grpo.pt", model, opt, 0, SFTConfig())
ptr = 0
for u in range(UPDATES):
    ...
    if u % 8 == 0 or u == UPDATES - 1:
        ...
        e10 = e6  # ROWS==6 here, so skip the redundant second full eval
        eval_win = e10["win"]
        flag = ""
        if e10["win"] > best:
            best = e10["win"]
            save_checkpoint("runs/cot_eph_aux_grpo.pt", model, opt, u + 1, SFTConfig())
            flag = "  <- best, saved"
...
load_checkpoint("runs/cot_eph_aux_grpo.pt", b)
